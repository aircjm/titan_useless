...
import com.aircjm.titan.mapper.auth.*;
import com.aircjm.titan.model.auth.*;
...
import com.aircjm.titan.service.auth.*;
...
public class AuthController extends BaseController {
    ...
    public RestResponse<List<GroupInfoResponse>> getGroupInfo() {
        Integer userId = 1;
        String projectType = "dice";
        return RestResponse.ok(authRoleMapper.getGroupInfo(userId, projectType));
    }



    @GetMapping(value = "/get_user_info")
    public RestResponse<UserInfoResponse> getUserInfo(@RequestParam(value = "token", required = false) String token) {
        if (StringUtils.isEmpty(token)) {
            token = request.getHeader(HttpHeaders.AUTHORIZATION);
        }
        return RestResponse.ok(userService.getUserInfo(token));
    }
