    ...
    public String selectMetaDtos(String type, String title, String snippetFileContent) {
        StringBuilder sql = new StringBuilder(
                "select t1.id, t1.name, t1.type, count(t3.id) as count\n" +
                        "from meta t1\n" +
                        "left join middle t2 on t2.m_id = t1.id ");
        if (null != snippetFileContent && !StringUtils.isEmpty(snippetFileContent)) {
            sql.append(" join (select id,title,type,status from article where deleted = 0 and id in (select snippet_id from dice.snippet_file where content like CONCAT('%',#{snippetFileContent},'%'))) t3 on t3.id = t2.a_id ");
        } else {
            sql.append(" left join article t3 on t3.id = t2.a_id and t3.deleted = 0");
        }
        switch (type) {
